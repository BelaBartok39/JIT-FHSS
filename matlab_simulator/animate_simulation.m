...
animData = struct();
animData.speedMultiplier = 50; % Start at 50x speed
animData.frameSkip = 5; % Update every 5 data points for smoother animation
...
beamCentralToSat = plot3(ax3d, [0, 0], [0, 0], [0, 0], 'c-', 'LineWidth', 2);
beamCentralToGS = plot3(ax3d, [0, 0], [0, 0], [0, 0], 'm-', 'LineWidth', 2);
beamSatToGS = plot3(ax3d, [0, 0], [0, 0], [0, 0], 'y-', 'LineWidth', 3);
...
while i <= length(times) && ishandle(fig)
    ...
    if ~animData.paused
        ...
        if visible
            % Alpha based on Doppler effect
            beamColor = [1, 1, 0, 0.3 + 0.6*dopplerAlpha]; % Yellow with Doppler-based alpha
            set(beamSatToGS, 'XData', [satPos(1), gsRadius], ...
                             'YData', [satPos(2), 0], ...
                             'ZData', [satPos(3), 0], ...
                             'Color', beamColor, 'LineWidth', 3);
        else
            % Invisible beam
        ...
        if visible
            if success
                set(txtSuccess, 'String', 'Status: ✓ DECODED', 'Color', 'g');
            else
                set(txtSuccess, 'String', 'Status: ✗ SYNC ERROR', 'Color', 'r');
            end
        else
            set(txtSuccess, 'String', 'Status: NOT VISIBLE', 'Color', [0.5, 0.5, 0.5]);
        end
